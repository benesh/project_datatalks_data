...
sys.path.insert(0, '/home/benomar/workspace/project_datatalks_data/prefect_flows_part/classes_models_services')
...
def fetching_file_with_wget(url_file, path_filename_compressed):
    try:
        tempvar = request.urlretrieve(url_file, path_filename_compressed)
...
def step_fetch_file_from_web(file_prop : FileProperties) -> None:
    ...
    fetching_file_with_wget(file_prop.url_file,
                            f'{file_prop.local_path_compressed}/{file_prop.file_name}.{file_prop.file_extension_compressed}')
    ...
    if len(list_of_element) > 1 :
        list_files,list_directories = get_files_folders_separate(list_of_element,
                                                                 file_prop.local_path_extracted)
        if list_files > 1 :
